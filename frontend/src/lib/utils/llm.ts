...
export interface LLMConfig {
	provider: 'openrouter' | 'custom' | 'openai' | 'azure';
	apiKey: string;
	modelName: string;
	baseURL: string;
	temperature: number;
...
export class LLMClient {
	...
	// Helper method to get the correct max tokens parameter
	private getMaxTokensParam(maxTokens: number): { [key: string]: number } {
		// Newer OpenAI models and Azure OpenAI use max_completion_tokens
	...
	async chatCompletion(systemPrompt: string, userPrompt: string, options?: any): Promise<{ content: string }> {
		const response = await this.client.chat.completions.create({
			...
			...this.getMaxTokensParam(options?.maxTokens ?? this.config.maxTokens),
			...options
		});
...
export function getLLMSettings(): LLMConfig {
	try {
		...
		if (providerType === 'openrouter') {
			...
			if (!openAIKey) {
				throw new Error('OpenAI API key not configured. Please go to Settings and add your OpenAI API key.');
			}
			...
			const azureSettings = settings.azure;
			
			if (!azureSettings?.endpoint) {
				throw new Error('Azure endpoint not configured. Please go to Settings and add your Azure endpoint.');
			}
			
			if (!azureSettings?.apiKey) {
				throw new Error('Azure API key not configured. Please go to Settings and add your Azure API key.');
			}
			
			if (!azureSettings?.deployment) {
				throw new Error('Azure deployment name not configured. Please go to Settings and add your deployment name.');
			}
			
			return {
				provider: 'azure',
				apiKey: azureSettings.apiKey,
				modelName: azureSettings.deployment,
				baseURL: azureSettings.endpoint,
				azureDeployment: azureSettings.deployment,
				azureApiVersion: azureSettings.apiVersion || '2024-12-01-preview',
				temperature: modelParams.temperature,
				maxTokens: modelParams.maxTokens
			};
